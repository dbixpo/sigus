...
def _ids_minhas_unidades():
    """IDs das unidades ativas do usuário logado."""
...
def _pode_ver_usuario(usuario):
    ...
    if current_user.perfil in ('gestor_secretaria', 'coordenador'):
        # Gestor Central e Coordenador vêem e editam todos (exceto administrador)
        return True
    if current_user.pode('cadastrar_usuario'):
        # Demais perfis: apenas usuários das próprias unidades
        meus_ids = set(_ids_minhas_unidades())
        ids_usuario = {uu.unidade_id for uu in usuario.unidades.filter_by(ativo=True).all()}
        return bool(meus_ids & ids_usuario)
    return False
...
def listar():
    ...
    if current_user.pode('gerenciar_usuarios') or current_user.perfil in ('gestor_secretaria', 'coordenador'):
        # Admin, gestor da secretaria e coordenador vêem todos, exceto administradores
        usuarios = Usuario.query.filter(
            Usuario.perfil != 'administrador'
        ).order_by(Usuario.nome).all()
    else:
        # Demais perfis: apenas usuários das suas próprias unidades (sem admins)
        from app.models.unidade import UsuarioUnidade
        meus_ids = _ids_minhas_unidades()
        ids_usuarios = db.session.query(UsuarioUnidade.usuario_id).filter(
            UsuarioUnidade.unidade_id.in_(meus_ids),
    ...
    return render_template('usuarios/listar.html', usuarios=usuarios, perfis=PERFIS,
...
@usuarios_bp.route('/<int:uid>/matriculas/<int:mid>', methods=['PUT'])
@login_required
